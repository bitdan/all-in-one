...
源码按职责分为 `activity`、`group`、`order`、`inventory`、`transaction` 和 `infrastructure`：
前四个包拥有各自的状态与持久化类型，`transaction` 负责跨域 Saga 和 PostgreSQL 本地事务编排，
`infrastructure` 放 Redis、Outbox、延迟任务与支付渠道适配器。HTTP API 路径未因内部拆包而变化。

退款网关当前使用 `LocalPaymentGateway` 模拟成功。接入真实支付渠道时，实现 `PaymentGateway`，并以
`paymentNo` 作为退款幂等键；支付回调入口还必须增加渠道验签或主动查询，不能直接信任请求中的金额和时间。
